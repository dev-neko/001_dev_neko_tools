...
def ipo_01(request):
	...
	for preurl in preurl_bbstr:
		...
		for fo_kanji in bs4obj_detail_url.find_all("td",attrs={'class':'main_data'}):
			...
			if (("証券" in kanji_text) or ("證券" in kanji_text))\
				...
				if kanji!=kanji_text:
					...
					# 先頭2文字だけ抽出
					elif kanji_text in "いちよし、マネックス、みずほ、岩井コスモ、東海東京、藍澤、むさし、":
						kanji_list.append("ユ"+kanji_text[0:2])
						kanji_list.append("ナ"+kanji_text[0:2])
					# 応募できないところは除外
					elif kanji_text in "エース、極東、西日本シティTT、あかつき、水戸、ゴールドマン・サックス"\
														 "J.Ｐ.モルガン、クレディ・スイス、豊証券、":
						pass
		...
		excel_data=excel_data+ipo_name+"\t"+\
							 datetime.datetime.strptime(bb_str,'%Y/%m/%d').strftime('%m/%d').lstrip("0").replace("/0","/")\
							 +get_day_of_week_jp(bb_str)+"\t"+\
							 datetime.datetime.strptime(bb_end,'%Y/%m/%d').strftime('%m/%d').lstrip("0").replace("/0","/")\
							 +get_day_of_week_jp(bb_end)+"\t"+\
							 datetime.datetime.strptime(bb_kekka,'%Y/%m/%d').strftime('%m/%d').lstrip("0").replace("/0","/")\
							 +get_day_of_week_jp(bb_kekka)+"\t"+\
							 kakaku
	...
	jinja_data={'gettime':excel_data,'limit':"auto"}
	data={'aaa':jinja_data,
				}
	template=loader.get_template('dev/jinja_template.html')
	return HttpResponse(template.render(data))
# IPOデータ取得してHTMLで表示 ここまで
# ------------------------------

def ysp_api_01(request):
