...
"""
def dev_input(request):
	# ここで初期化しないとリロードしても検索条件消えない
	dt_read_db_data=[]
	dt_exists_db_data={'exists_db_data':BookModel.objects.all()}
	# print(db_id)
	...
	if request.method=='POST':
		...
		if request.POST["db_action_btn"]=="save":
			...
			post_radio=request.POST["exclude_id"]
			# idを指定しないと最後に追加される
			BookModel.objects.create(bookname=post_src_url,summary="aaa",rating="4",radio=post_radio)
			# 登録後は最新のDBの内容を読み込んでDjangoテンプレートに渡す
			dt_read_db_data={'read_db_data':BookModel.objects.order_by("id").last()}
		elif request.POST["db_action_btn"]=="read":
			dt_read_db_data={'read_db_data':BookModel.objects.get(id=request.POST["select_db_data"])}
		elif request.POST["db_action_btn"]=="delete":
			BookModel.objects.filter(id=request.POST["select_db_data"]).delete()
	...
	return render(request, 'work_apps/tame_input.html', dt_data)

def dev_output(request):
	django_template_data={}
	print(request.POST)
	return render(request, 'work_apps/tame_output.html', django_template_data)
"""
